...
export default function RecognisedBy() {
  return (
    <section style={{ background: '#D4EAF8', borderBottom: '3px solid #1A1A1A', padding: '60px 0' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 32px' }}>
        <div className="flex flex-col md:flex-row items-center justify-center gap-10 md:gap-16">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', gap: '32px 44px' }}>
            {logos.map((logo) => (
              <img key={logo.alt} src={logo.src} alt={logo.alt} className={`${logo.className} w-auto object-contain transition-all duration-200 hover:scale-110`} />
            ))}
          </div>
        </div>
      </div>
